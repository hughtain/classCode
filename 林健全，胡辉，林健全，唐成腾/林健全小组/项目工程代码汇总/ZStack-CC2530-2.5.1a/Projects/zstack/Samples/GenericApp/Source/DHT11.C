...
{     
    ...
    for(i=0;i<8;i++)    
    {
     ...
     Delay_10us();
     uchartemp=0;
     if(wenshi)uchartemp=1;//如果超过30us还没置0，则数据为1，置0，则数据为0
     ucharFLAG=2;
     while((wenshi)&&ucharFLAG++);//等待置0    
...
void DHT11_TEST(void)   //温湿传感启动
{
    wenshi=0;//置P0_7低电平
    Delay_ms(19);  //>18MS
    wenshi=1; //置P0_7高电平，开始信号发送结束
    P0DIR &= ~0x40; //重新配置IO口方向
    Delay_10us();
    Delay_10us();						
    Delay_10us();
    Delay_10us();  //高电平，延时40us等待
     if(!wenshi)  //如果电平变低，则证明检测到开始信号
     {
      ucharFLAG=2; 
      while((!wenshi)&&ucharFLAG++);//低电平，等待响应结束
      ucharFLAG=2;
      while((wenshi)&&ucharFLAG++); //高电平，准备输出
      COM();
